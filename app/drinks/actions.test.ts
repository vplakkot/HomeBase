...
function given(permissions = ["use_modules"]) {
  fake = fakeSupabase({ permissions, tables: { drinks: [{ id: DRINK }], drink_ratings: [] } });
...
describe("adding a drink by hand (REQ-37)", () => {
  it("saves one with only a name and opens it", async () => {
    given();
    await expect(addDrink({}, form({ name: "House red" }))).rejects.toThrow(`REDIRECT:/drinks/${DRINK}`);
  ...
  it("is refused to someone who can't use modules", async () => {
    given([]);
    await expect(addDrink({}, form({ name: "House red" }))).rejects.toThrow("REDIRECT:/drinks");
  });

  it("changes a drink's details and removes one", async () => {
    given();
    await expect(updateDrink({}, form({ id: DRINK, name: "Renamed" }))).rejects.toThrow(`REDIRECT:/drinks/${DRINK}`);
...
describe("rating a drink (REQ-29)", () => {
  it("saves whole stars and a comment as the signed-in person's own rating, replacing any before", async () => {
    ...
    expect(ratings.upsert).toHaveBeenCalledWith(
      { drink_id: DRINK, user_id: "user-1", stars: 4, comment: "great with lamb" },
