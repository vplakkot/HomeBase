...
// The hourly schedules' addresses are next (the test notification and the
// Finances reminders): they run in the database with nobody signed in,
// and prove themselves with a shared secret instead. Sent through here
// they would be answered with the sign-in page, and the schedule would
...
export const config = {
  matcher: [
    "/((?!_next/static|_next/image|favicon.ico|manifest\\.webmanifest$|sw\\.js$|api/notifications/(?:test|receipt|finances)$|.*\\.(?:svg|png|jpg|jpeg|gif|webp|ico)$).*)",
  ],
};
